...
void _order(avl_t **root, int *array, size_t lo, size_t hi)
{
	...
	if (hi - lo > 1)
	{
		...
		if (array[middle] > (*root)->n)
			...
			(*root)->left = new;
		_order(&new, array, lo, middle);
		_order(&new, array, middle, hi);
	}
}
...
avl_t *sorted_array_to_avl(int *array, size_t size)
{
	...
	_order(&tree, array, -1, middle);
	_order(&tree, array, middle, size);
